@everywhere setupcode = quote
  ...
  endtime = DateTime(2015,07,03,12,00,00)
  ...
  function peturbsample(M,N,deps,phase)
    DiscreteMaps.timedsample("Lh",endtime=endtime,NP=M,NCycles=1,startstring="results/lrb/rb-$(M)-$(N)-$(deps)-$(round(phase/2pi,3))",
                             Itargs=(3.8,phase),Itkwargs=DiscreteMaps.kw(samplefn=DiscreteMaps.evensamplefn,samplefnargs=(deps),N=N))
  end
end
...
(length(ARGS) == 1) ? (M = int(ARGS[1])) : (M = 20)
 while (now() < endtime)
  for N in [10^8], deps in ([1:1000]*1e-7), phase in (2pi * [0:29]/30)
